...
int oplus_usb_get_property(struct power_supply *psy,
	...
	/* OuYangBaiLi@BSP.CHG.Basic,charging_bsp 2021/04/21 */
	if (!g_charger_chip) {
		return ret;
	}
	/* OuYangBaiLi@BSP.CHG.Basic,charging_bsp 2021/04/21 */
		...
		const union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_EM_MODE:
			chip->em_mode = val->intval;
			pr_err("************* write em_mode = %d\n", chip->em_mode);
			if(chip->em_mode)
				chip->chg_ops->em_mode_enable();
			break;
...
#endif
		case POWER_SUPPLY_PROP_EM_MODE:
			val->intval = chip->em_mode;
			pr_err("************* read em_mode = %d\n", chip->em_mode);
			break;
		case POWER_SUPPLY_PROP_SUB_CURRENT:
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	if (get_eng_version() == HIGH_TEMP_AGING) {
		chg_err(" HIGH_TEMP_AGING ,hot_bat_decidegc to 99 \n");
...
static void oplus_chg_set_charging_current(struct oplus_chg_chip *chip)
{
	...
	charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY] charging current: %d\n", charging_current);
...
static void oplus_chg_check_ffc_temp_status(struct oplus_chg_chip *chip)
{
	...
	if (temp_status != chip->ffc_temp_status) {
		...
		if (chip->ffc_temp_status == FFC_TEMP_STATUS__NORMAL || chip->ffc_temp_status == FFC_TEMP_STATUS__WARM) {
...
static bool oplus_chg_check_tbatt_is_good(struct oplus_chg_chip *chip)
{
	...
	if (batt_temp > chip->limits.hot_bat_decidegc || batt_temp < chip->limits.cold_bat_decidegc) {
		...
		if (temp_counts >= TEMP_CNT) {
			...
			if (is_project(20615) || is_project(20662)) {
				if (batt_temp < -50 && chip->usb_temp_r >= 0 && is_vooc_support_single_batt_svooc() == true) {	/*-77 remove*/
					tbatt_status = BATTERY_STATUS__REMOVED;
				}
			}
		}
	...
	if (chip->chg_ops->check_chrdet_status()) {
		...
		if (chip->charger_type == POWER_SUPPLY_TYPE_UNKNOWN) {
			...
			charger_xlog_printk(CHG_LOG_CRTI, "Charger in 1 charger_type=%d\n",
				chip->charger_type);
			power_supply_changed(chip->usb_psy);/*for update state*/
			if (oplus_vooc_get_fastchg_to_normal() == true
					|| oplus_vooc_get_fastchg_to_warm() == true) {
				charger_xlog_printk(CHG_LOG_CRTI,
...
static bool oplus_chg_soc_reduce_slow_when_1(struct oplus_chg_chip *chip)
{
	...
	if (chip->batt_volt_min < chip->vbatt_soc_1) {
		...
		reduce_count = 0;
		if (get_eng_version() == HIGH_TEMP_AGING && chip->ui_soc <= 2) {
			chip->ui_soc = 2;
			chip->soc = 2;
		}
	}
...
int oplus_chg_fast_switch_check_flag = 0;
static void oplus_chg_qc_config(struct oplus_chg_chip *chip);
static void oplus_chg_fast_switch_check(struct oplus_chg_chip *chip)
{
	...
	if (get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT || get_boot_mode() == LOW_POWER_OFF_CHARGING_BOOT) {
		if(oplus_chg_fast_switch_check_flag == 0) {
			printk("oplus_chg_fast_switch_check_flag set 1 for power off fast charge delay 10s\n");
			msleep(10000);
			oplus_chg_fast_switch_check_flag = 1;
		}
	}
#ifndef CONFIG_OPLUS_CHARGER_MTK
#ifdef SUPPORT_WPC
...
int oplus_chg_match_temp_for_chging(void)
{
	...
	pr_err("%s: batt_temp :%d, shell_temp :%d\n", __func__, batt_temp, shell_temp);

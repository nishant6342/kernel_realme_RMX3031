...
extern bool oplus_gauge_get_batt_external_hmac(void);
#endif
...
bool set_charge_power_sel(enum CHARGE_SEL select)
{
	...
	//gm = get_mtk_battery();
	pr_err(" set_charge_power_sel select = %d\n",select);
...
EXPORT_SYMBOL(set_charge_power_sel);
...
{
	int bat_volt = meter_fg_30_get_battery_mvolts();
	if (oplus_gauge_get_batt_external_hmac() == false && ( bat_volt > 3650 ) && (bat_volt < 3950)) {
		gm.ui_soc = 55;
		gauge_dev_set_rtc_ui_soc(gm.gdev, gm.ui_soc);
...
bool meter_fg_30_get_battery_authenticate(void)
{
    #ifdef CONFIG_OPLUS_4400MV_BATTERY_SUPPORT
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (oplus_gauge_get_batt_external_hmac() == false) {
		return false;
	}
...
/*Baoquan.Lai@BSP.CHG.Basic 2020/11/23 add for distinguish battery*/
	...
	pr_err("%s, is_4450mv_battery_support:%d\n", __func__, is_4450mv_battery_support);
/*Baoquan.Lai@BSP.CHG.Basic 2020/11/25 add for force full*/
	...
	pr_err("%s, enable_is_force_full:%d\n", __func__, enable_is_force_full);
/*Baoquan.Lai@BSP.CHG.Basic 2020/12/25 add for force full*/
	...
	pr_err("%s, is_subboard_temp_support:%d\n", __func__, is_subboard_temp_support);
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
	if(is_fuelgauge_apply() == true) {

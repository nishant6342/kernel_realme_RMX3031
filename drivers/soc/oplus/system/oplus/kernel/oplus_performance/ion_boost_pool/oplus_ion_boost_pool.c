...
static int fill_boost_page_pool(struct device *dev, struct ion_msm_page_pool *pool)
{
	struct page *page;
					 ...
					 char *name, int cached)
{
	...
	boost_pool->proc_low_info = proc_create_data(buf, 0666,
						 ...
						 boost_pool);
	if (IS_ERR(boost_pool->proc_low_info)) {
		pr_info("Unable to initialise /proc/boost_pool/%s_low\n",
			name);
		goto destroy_proc_info;
	} else {
		pr_info("procfs entry /proc/boost_pool/%s_low allocated. data=%p\n",
			name, boost_pool);
	}

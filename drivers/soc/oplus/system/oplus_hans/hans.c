		...
		int target_pid, int target_uid, const char *rpc_name, int code)
{
	...
	ret = genl_msg_prepare_usr_msg(HANS_CMD_GENL, size,
				       (pid_t)atomic_read(&hans_deamon_port), &skb);
	...
	if ((ret = genlmsg_unicast(&init_net, skb,
				   (u32)atomic_read(&hans_deamon_port))) < 0) {
...
int register_hans_vendor_hooks(void)
{
	...
	rc = register_trace_android_vh_binder_alloc_new_buf_locked(binder_alloc_handler,
			NULL);
...
void unregister_hans_vendor_hooks(void)
{
	...
	unregister_trace_android_vh_binder_alloc_new_buf_locked(binder_alloc_handler,
			NULL);

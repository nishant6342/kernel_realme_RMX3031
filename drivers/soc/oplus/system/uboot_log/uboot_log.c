...
static int uboot_kboot_buffer_init()
{
	...
	kboot_uboot_logmemory = of_get_child_by_name(reserved_memory,
				"kboot_uboot_logmem");
	...
	if (of_property_read_u32(kboot_uboot_logmemory, "xbluefi-offset",
				 &xbl_uefi_logoffset)) {
...
int ubootback_thread_fn(void *data)
{
	...
	while (kboot_log_buf_len > total_size + 2 * LINE_MAX_SIZE) {
		...
		if (back_kmsg_dump_get_buffer(&kboot_dumper, true, kboot_log_buf + total_size,
					      kboot_log_buf_len - total_size, &line_len)) {
			total_size += line_len;
			line_len = 0;
		}

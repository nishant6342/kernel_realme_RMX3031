...
static int oplus_memcfg_slabtrace_show(struct seq_file *m, void *p)
{
	...
	pid_cnt_cachep = kmem_cache_create("pid_cnt_cache",
					   sizeof(struct pid_cnt_struct),
					   0, 0, NULL);
...
static int oplus_memcfg_slabcfg_show(struct seq_file *m, void *p)
{
	...
	seq_printf(m,
		   "Print type:echo \"print symbol/address\" > /proc/slab_memcfg/slabcfg\n");
	seq_printf(m,
		   "Specific kmalloc:echo \"kmalloc-x pid\" > /proc/slab_memcfg/slabcfg\n");
	seq_printf(m,
		   "Specific kmalloc sort by stacks:echo \"kmalloc-x stacks add1 add2 ....\" > /proc/slab_memcfg/slabcfg\n");
...
static ssize_t slabcfg_write(struct file *filp, const char *ubuf, size_t cnt,
			     loff_t *data)
{
	char buf[1024] = {};
	char mchar[] = "kmalloc-";
	...
	sscanf(buf, "%s %s %lx %lx %lx %lx", argv1, argv2, &sort_addrs[0],
	       &sort_addrs[1], &sort_addrs[2], &sort_addrs[3]);
	pr_info("slabtrace:%s %s %lx %lx %lx %lx\n", argv1, argv2, sort_addrs[0],
		sort_addrs[1], sort_addrs[2], sort_addrs[3]);
...
static int __init oplus_memcfg_late_init(void)
{
	...
	if (!oplus_memcfg_dir) {
		...
		pEntry = proc_create("slabtrace", 0400, oplus_memcfg_dir,
		...
		pEntry = proc_create("slabcfg", 0600, oplus_memcfg_dir,
				     &proc_slabcfg_operations);

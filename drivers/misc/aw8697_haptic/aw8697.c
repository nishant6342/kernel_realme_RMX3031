...
static int aw8697_haptic_rtp_init(struct aw8697 *aw8697)
{
    ...
    while((!aw8697_haptic_rtp_get_fifo_afs(aw8697)) &&
        ...
        if((aw8697->rtp_cnt >= aw8697_rtp->len) || (aw8697->ram.base_addr == 0) || ((glb_state_val & 0x0f) == 0x00)) {
            pr_info("%s: rtp update complete, aw8697->ram.base_addr[%d], glb_state_val = 0x%x\n", __func__, aw8697->ram.base_addr, glb_state_val);
            aw8697->rtp_cnt = 0;
            aw8697_dump_rtp_regs(aw8697);
            mutex_unlock(&aw8697->rtp_lock);

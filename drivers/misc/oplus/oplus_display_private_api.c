...
bool oppo_display_cabc_support; /* if cabc is supported(cmd sent without cmdq handle) */
bool oppo_display_cabc_cmdq_support; /* if cabc is supported(cmd sent with cmdq handle) */
...
bool oppo_display_aod_support; /* if aod feature is supported */
...
#if 0
bool oppo_display_lcm_id_check_support; /* whether lcm id is needed to be checked or not(only himax ic of 18311&18011 for now) */
...
u32 oppo_display_mipi_before_init = 0; /* frame data must be sent before lcd init if value is 1(ic:TM TD4330) */
...
u32 oppo_display_bl_set_on_lcd_esd_recovery = 0;
/* need recover backlight after esd recovery if value is 1(cmd sent with cmdq handle) */
u32 oppo_display_bl_set_cmdq_on_lcd_esd_recovery = 0;
...
u32 oppo_display_esd_try_count = 0; /* customize esd consecutive recover max time until success (only 18531&18561&18161 for now)*/
        ...
        struct device_attribute *attr, const char *buf, size_t num)
{
    ...
    */
    if (oppo_display_cabc_support || oppo_display_cabc_cmdq_support) {

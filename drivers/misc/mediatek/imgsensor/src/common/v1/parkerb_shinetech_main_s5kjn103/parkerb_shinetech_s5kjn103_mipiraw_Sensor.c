...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
#define DEVICE_VERSION_S5KJN103	 "parkerb_shinetech_s5kjn103"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = PARKERB_SHINETECH_MAIN_S5KJN103_SENSOR_ID_ORIGINAL,
			...
			*sensor_id = (((read_cmos_sensor_8(0x0000) << 8)
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(deviceInfo_register_value == 0x00){
					Eeprom_DataInit(0, PARKERB_SHINETECH_MAIN_S5KJN103_SENSOR_ID);
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable)
		write_cmos_sensor(0x0600, 0x0001); /* ANDROID_SENSOR_TEST_PATTERN_MODE_BLACK */
	else

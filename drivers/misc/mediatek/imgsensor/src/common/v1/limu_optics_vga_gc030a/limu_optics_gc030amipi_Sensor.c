...
#include "limu_optics_gc030amipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "limu_gc030a_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = LIMU_OPTICS_VGA_GC030A_SENSOR_ID,
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		...
		write_cmos_sensor(0xfe, 0x03);
		write_cmos_sensor(0x10, 0x90);
		write_cmos_sensor(0xfe, 0x00);/*Stream on */
	} else {
		/* MIPI */
		write_cmos_sensor(0xfe, 0x03);
		write_cmos_sensor(0x10, 0x00);
		write_cmos_sensor(0xfe, 0x00);/*Stream off */
	}

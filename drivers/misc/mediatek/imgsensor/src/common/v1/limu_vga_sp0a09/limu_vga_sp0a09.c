...
#include "limu_vga_sp0a09.h"



/****************************Modify Following Strings for Debug****************************/
#define PFX "LIMU_VGA_SP0A09_camera_sensor"
#define LOG_1 LOG_INF("LIMU_VGA_SP0A09,MIPI 1LANE\n")
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = LIMU_VGA_SP0A09_SENSOR_ID,
   	 ...
   	 {   
            ...
            //   iReg+= (gain%BASEGAIN)/(0x10/BASEGAIN);
   	    	 ...
   	    	 if(iReg<=0x10)
   	    	 {
   	    	    	 ...
   	    	    	 write_cmos_sensor(0x01, 0x01);
                      LOG_INF("LIMU_VGA_SP0A09MIPI_SetGain = 16");
   	    	 }
   	    	 else if(iReg>= 0xa0)//gpw
   	    	 {
   	    	    	 ...
   	    	    	 write_cmos_sensor(0x01, 0x01); 
                      LOG_INF("LIMU_VGA_SP0A09MIPI_SetGain = 160"); 
	        }        	
   	    	 else
   	    	 {
   	    	    	write_cmos_sensor(0xfd, 0x00);
   	    	    	...
   	    	    	write_cmos_sensor(0x01, 0x01);
			LOG_INF("LIMU_VGA_SP0A09MIPI_SetGain = %d",iReg);		 
	       }	
   	 }	
   	 else
...
static void sensor_init(void)
{
   ...
   write_cmos_sensor(0x13,0x18); //修改为默认值
   ...
   write_cmos_sensor(0x70,0x22); //时序的修改
...
static void preview_setting(void)
{
    /********************************************************
       *
       *   640x480 30fps 2 lane MIPI 120Mbps/lane
       *
       ********************************************************/
//    write_cmos_sensor(0xfd,0x00);
...
static kal_uint32 streaming_control(kal_bool enable)
{
    ...
    if (enable) {
        /* MIPI */
        write_cmos_sensor(0xfd, 0x00);
        write_cmos_sensor(0xa4, 0x01); /*Stream on */
    } else {
        /* MIPI */
        write_cmos_sensor(0xfd, 0x00);
        write_cmos_sensor(0xa4, 0x00); /*Stream off */
    }

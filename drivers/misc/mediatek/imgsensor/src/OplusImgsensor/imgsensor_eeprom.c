                ...
                ACDK_SENSOR_ENGMODE_STEREO_STRUCT *pStereoData)
{
    ...
    switch (uSensorId) {
        ...
        case IMX682_SENSOR_ID_20615:
        {
            if (data_length == CALI_DATA_MASTER_LENGTH_20615
                && data_base == IMX682_STEREO_START_ADDR_20615) {
                pr_debug("imx682 DA:0x%x DL:%d", data_base, data_length);
            } else {
                pr_debug("imx682 Invalid DA:0x%x DL:%d", data_base, data_length);
                return IMGSENSOR_RETURN_ERROR;
            }
            break;
        }
        ...
        case HI846_SENSOR_ID_20615:
        {
            if (data_length == CALI_DATA_SLAVE_LENGTH_20615
                && data_base == HI846_STEREO_START_ADDR_20615) {
...
void Eeprom_StereoDataRead(enum IMGSENSOR_SENSOR_IDX sensor_idx, kal_uint16 slaveAddr)
{
    ...
    if (pCamDeviceObj->i4MWDataIdx == IMGSENSOR_SENSOR_IDX_MAIN2) {
        if (sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN) {
            if ((pCamDeviceObj->i4CurSensorId == IMX682_SENSOR_ID_20615) || (pCamDeviceObj->i4CurSensorId == OV64B_SENSOR_ID_20619)) {
            ...
            if ((pCamDeviceObj->i4CurSensorId == HI846_SENSOR_ID_20615) || (pCamDeviceObj->i4CurSensorId == HI846_SENSOR_ID_20619)) {
            ...
            kal_uint32 sensorID)
{
    ...
    switch (feature_id) {
        case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        {
            ...
            if(i4Sensor_idx == 2){
                Dac_inf = pCamDeviceObj->camNormdata[0][28+6] + (pCamDeviceObj->camNormdata[0][28+7] << 8);
                Dac_mac = pCamDeviceObj->camNormdata[0][28] + (pCamDeviceObj->camNormdata[0][28+1] << 8);
                Dac_master = (5*Dac_mac+36*Dac_inf)/41+30;
                memcpy(&feature_para[28], (unsigned char*)&Dac_master, 4);
            }
            break;
        }
        case SENSOR_FEATURE_GET_EEPROM_STEREODATA:
        {
            ...
            if (i4Sensor_idx == pCamDeviceObj->i4MWDataIdx) {
                if ((pCamDeviceObj->i4CurSensorId == HI846_SENSOR_ID_20615) || (pCamDeviceObj->i4CurSensorId == HI846_SENSOR_ID_20619)) {
                    memcpy(&feature_para[0], pCamDeviceObj->stereoMWdata, DUALCAM_CALI_DATA_LENGTH_TOTAL_20615);
                } else {

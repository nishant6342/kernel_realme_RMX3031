...
#define OPPO_MAX_QRTR_SERVICE_LEN         120

atomic_t qrtr_first_msg = ATOMIC_INIT(0);
u64 oppo_nw_wakeup[OPPO_NW_WAKEUP_SUM] = {0};
static u64 service_wakeup_times[OPPO_MAX_QRTR_SERVICE_LEN][4] = {{0}};

/*Add for ipa wakeup msg*/
#define OPPO_MAX_RECORD_IP_LEN             60
#define OPPO_TRANSMISSION_INTERVAL         3 * 1000
#define OPPO_TCP_RETRANSMISSION_INTERVAL   1 * 1000
#define OPPO_MAX_RECORD_APP_WAKEUP_LEN     100
...
struct tcp_hook_struct {
	...
	u64 ipv6_addr1;
	u64 ipv6_addr2;
	u64 set[OPPO_MAX_RECORD_IP_LEN*3];
};

struct tcp_hook_simple_struct {
	u32 count;
	u64 set[OPPO_MAX_RECORD_APP_WAKEUP_LEN*3+1];
};
...
u64 oppo_mdaci_nw_wakeup[OPPO_NW_WAKEUP_SUM] = {0};
static u64 mdaci_service_wakeup_times[OPPO_MAX_QRTR_SERVICE_LEN][4] = {{0}};
...
#define OPPO_MAX_RECORD_BLACK_REJECT_LEN 100
...
static void nwpower_unsl_blacklist_reject() {
	if (record_blacklist_reject_index > 0) {
		...
		memset(blacklist_reject_uid, 0x0, OPPO_MAX_RECORD_BLACK_REJECT_LEN * sizeof(u32));
	...
	if (blacklist_len > 0 && sock && sock->sk && (sock->sk->sk_family == 2 || sock->sk->sk_family == 10)) {
		...
		for (i = 0; i < blacklist_len; i++) {
			if(blacklist_uid[i] == uid) {
				if (is_input != 1) {
					if (record_blacklist_reject_index >= OPPO_MAX_RECORD_BLACK_REJECT_LEN) {
				...
				printk("[oppo_netcontroller] blacklist reject, stamp=%ld, is_input=%d, uid=%u",
...
static int nwpower_set_blacklist_uids(struct nlmsghdr *nlh) {
	...
	for (i = 0; i < blacklist_len; i++) {
		if (i >= KERNEL_UNSL_APP_WAKEUP_LEN) {
			printk("[oppo_netcontroller] uids length exceed the limit!");
			break;
		} else {
...
extern void oppo_match_modem_wakeup() {
	atomic_set(&qrtr_first_msg, 1);
	oppo_nw_wakeup[OPPO_NW_MPSS]++;
	oppo_mdaci_nw_wakeup[OPPO_NW_MPSS]++;
}

extern void oppo_match_wlan_wakeup() {
	oppo_nw_wakeup[OPPO_NW_WIFI]++;
	oppo_mdaci_nw_wakeup[OPPO_NW_WIFI]++;
}

static void match_qrtr_new_service_port(int id, int port, u64 qrtr[][4]) {
	int i;
	for (i = 0; i < OPPO_MAX_QRTR_SERVICE_LEN; ++i) {
		if (qrtr[i][0] == 0) {
			qrtr[i][0] = 1;
			qrtr[i][1] = id;
			qrtr[i][2] = port;
...
static void __oppo_match_qrtr_wakeup(int src_node, int src_port, int dst_port,
	unsigned int arg1, unsigned int arg2, u64 qrtr[][4], u64 wakeup[OPPO_NW_WAKEUP_SUM], bool prt) {
	int i;
	int repeat[4] = {0};
	int repeat_index = 0;
...
extern void oppo_match_qrtr_wakeup(int src_node, int src_port, int dst_port, unsigned int arg1, unsigned int arg2) {
	__oppo_match_qrtr_wakeup(src_node, src_port, dst_port, arg1, arg2, service_wakeup_times, oppo_nw_wakeup, true);
	__oppo_match_qrtr_wakeup(src_node, src_port, dst_port, arg1, arg2, mdaci_service_wakeup_times, oppo_mdaci_nw_wakeup, false);
	atomic_set(&qrtr_first_msg, 0);
}

extern void oppo_update_qrtr_flag(int flag) {
	if (atomic_read(&qrtr_wakeup_hook_boot) == 1) {
		atomic_set(&qrtr_first_msg, flag);
	}
}

static void print_qrtr_wakeup(bool unsl, u64 qrtr[][4], u64 wakeup[OPPO_NW_WAKEUP_SUM], bool prt) {
	u64 temp[5][4] = {{0}};
	u64 max_count = 0;
	u64 max_count_id = 0;
	int j;
	...
	if (unsl) {
		wakeup_unsl_msg[5] = (wakeup[OPPO_NW_MD] << 48) |
			((wakeup[OPPO_NW_QRTR] & 0xFFFF) << 32) |
			((wakeup[OPPO_NW_WIFI] & 0xFFFF) << 16) |
			(wakeup[OPPO_NW_MPSS] & 0xFFFF);
	}
	if (prt)
		printk("[oppo_nwpower] AllWakeups: Mpss: %d, Qrtr: %d, Modem: %d, WiFi: %d",
			wakeup[OPPO_NW_MPSS], wakeup[OPPO_NW_QRTR], wakeup[OPPO_NW_MD], wakeup[OPPO_NW_WIFI]);
}
	...
	if (atomic_read(&ipa_wakeup_hook_boot) == 1) {
		if (atomic_read(&tcp_is_input) == 0) {
			...
			if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) - (tcp_last_transmission_stamp.tv_sec * 1000 + tcp_last_transmission_stamp.tv_nsec / 1000000))
				> OPPO_TRANSMISSION_INTERVAL) {
				if (type == OPPO_TCP_TYPE_V4) {
					tmp_v4iph = ip_hdr(skb);
					atomic_set(&tcp_is_input, OPPO_TCP_TYPE_V4);
					...
					atomic_set(&tcp_is_input, OPPO_TCP_TYPE_V6);
	...
	if (atomic_read(&ipa_wakeup_hook_boot) == 1) {
		if (atomic_read(&tcp_is_input) == type && !tcp_input_sch_work) {
			if (sk->sk_state != TCP_TIME_WAIT) {
				...
				tcp_input_list.pid = sk->sk_oppo_pid;
		...
		sk->oppo_last_rcv_stamp[1] = tcp_last_transmission_stamp.tv_sec * 1000 + tcp_last_transmission_stamp.tv_nsec / 1000000;
	}
}

extern void oppo_ipa_schedule_work() {
	...
	if (atomic_read(&ipa_wakeup_hook_boot) == 1) {
		if (atomic_read(&tcp_is_input) == 0) {
			...
			if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) - (tcp_last_transmission_stamp.tv_sec * 1000 + tcp_last_transmission_stamp.tv_nsec / 1000000))
				...
				tcp_output_list.pid = sk->sk_oppo_pid;
		...
		sk->oppo_last_send_stamp[1] = tcp_last_transmission_stamp.tv_sec * 1000 + tcp_last_transmission_stamp.tv_nsec / 1000000;
	}
}
...
extern void oppo_match_tcp_input_retrans(struct sock *sk) {
	...
	if (atomic_read(&tcpsynretrans_hook_boot) == 1) {
		...
		if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) - sk->oppo_last_rcv_stamp[0]) > OPPO_TCP_RETRANSMISSION_INTERVAL) {
			...
			tcp_input_retrans_list.pid = sk->sk_oppo_pid;
...
extern void oppo_match_tcp_output_retrans(struct sock *sk) {
	...
	if (atomic_read(&tcpsynretrans_hook_boot) == 1) {
		...
		if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) - sk->oppo_last_send_stamp[0]) > OPPO_TCP_RETRANSMISSION_INTERVAL) {
			...
			tcp_output_retrans_list.pid = sk->sk_oppo_pid;
...
static void tcp_hook_insert_sort(struct tcp_hook_struct *pval) {
	...
	u64 count = 0;
	u64 temp_sort[3] = {0};
	for (i = 1; i < OPPO_MAX_RECORD_IP_LEN; ++i) {
...
static int match_tcp_hook(struct tcp_hook_struct *pval) {
	...
	for (i = 0; i < OPPO_MAX_RECORD_IP_LEN; ++i) {
		if (pval->is_ipv6) {
...
static void app_wakeup_monitor(bool is_block, bool is_input, int pid, int uid) {
	...
	if (app_wakeup_monitor_list.count < OPPO_MAX_RECORD_APP_WAKEUP_LEN) {
		app_wakeup_monitor_list.set[1+app_wakeup_monitor_list.count*3] = (u64)pid << 32 | uid;
		app_wakeup_monitor_list.set[1+app_wakeup_monitor_list.count*3+1] =
			((u64)block << 48) |
			((u64)(input & 0xFFFF) << 32) |
			((u64)(whitelist & 0xFFFF) << 16) |
			(0xFFFF);
		app_wakeup_monitor_list.set[1+app_wakeup_monitor_list.count*3+2] = now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000;
		/*
		printk("[oppo_nwpower] count:%d, block:%d, input:%d, pid:%d, uid:%d, stamp:%ld",
			app_wakeup_monitor_list.set[0], block, input, pid, uid, app_wakeup_monitor_list.set[1+app_wakeup_monitor_list.count*3+2]);
		*/
		app_wakeup_monitor_list.set[0] = (++app_wakeup_monitor_list.count);
	} else {
		printk("[oppo_nwpower] warning! OPPO_MAX_RECORD_APP_WAKEUP_LEN reached");
	}
}
...
static void tcp_output_tcpsynretrans_hook_work_callback(struct work_struct *work) {
	...
	if (tcp_output_retrans_list.is_ipv6) {
		...
		printk("[oppo_nwpower] TCPOutputRetrans: [%ld,%ld], %d, %d, %d",
			tcp_output_retrans_list.ipv6_addr1, tcp_output_retrans_list.ipv6_addr2,
		...
		printk("[oppo_nwpower] TCPOutputRetrans: %d, %d, %d, %d",
			tcp_output_retrans_list.ipv4_addr, tcp_output_retrans_list.pid, tcp_output_retrans_list.uid,
...
static void tcp_input_tcpsynretrans_hook_work_callback(struct work_struct *work) {
	...
	if (tcp_input_retrans_list.is_ipv6) {
		...
		printk("[oppo_nwpower] TCPInputRetrans: [%ld,%ld], %d, %d, %d",
			tcp_input_retrans_list.ipv6_addr1, tcp_input_retrans_list.ipv6_addr2,
		...
		printk("[oppo_nwpower] TCPInputRetrans: %d, %d, %d, %d",
			tcp_input_retrans_list.ipv4_addr, tcp_input_retrans_list.pid, tcp_input_retrans_list.uid,
...
static void print_ipa_wakeup(bool unsl, struct tcp_hook_struct *ptcp_in, struct tcp_hook_struct *ptcp_out,
	struct tcp_hook_struct *ptcp_re_in, struct tcp_hook_struct *ptcp_re_out, u64 wakeup[OPPO_NW_WAKEUP_SUM], bool prt) {
	wakeup[OPPO_NW_TCP_IN] = print_tcp_wakeup("Input", ptcp_in, prt);
	wakeup[OPPO_NW_TCP_OUT] = print_tcp_wakeup("Output", ptcp_out, prt);
	wakeup[OPPO_NW_TCP_RE_IN] = print_tcp_wakeup("InputRetrans", ptcp_re_in, prt);
	wakeup[OPPO_NW_TCP_RE_OUT] = print_tcp_wakeup("OutputRetrans", ptcp_re_out, prt);
	if (prt)
		printk("[oppo_nwpower] IPAAllWakeups: TCPInput: %d, TCPOutput: %d, TCPInputRetrans: %d, TCPOutputRetrans: %d",
			wakeup[OPPO_NW_TCP_IN], wakeup[OPPO_NW_TCP_OUT], wakeup[OPPO_NW_TCP_RE_IN], wakeup[OPPO_NW_TCP_RE_OUT]);
	if (unsl) {
		wakeup_unsl_msg[6] = (wakeup[OPPO_NW_TCP_IN] << 48) |
			((wakeup[OPPO_NW_TCP_OUT] & 0xFFFF) << 32) |
			((wakeup[OPPO_NW_TCP_RE_IN] & 0xFFFF) << 16) |
			(wakeup[OPPO_NW_TCP_RE_OUT] & 0xFFFF);
	}
}

static void reset_count(u64 qrtr[][4], struct tcp_hook_struct *ptcp_in, struct tcp_hook_struct *ptcp_out,
	struct tcp_hook_struct *ptcp_re_in, struct tcp_hook_struct *ptcp_re_out, u64 wakeup[OPPO_NW_WAKEUP_SUM]) {
	int i;
	int j;
	for (i = 0; i < OPPO_MAX_QRTR_SERVICE_LEN; ++i) {
		if (qrtr[i][0] == 1) {
	...
	for (i = 0; i < OPPO_NW_WAKEUP_SUM; ++i) {
		wakeup[i] = 0;
	}
}
...
static void nwpower_hook_off(bool unsl) {
	...
	print_qrtr_wakeup(unsl, service_wakeup_times, oppo_nw_wakeup, true);
	print_ipa_wakeup(unsl, &tcp_input_list, &tcp_output_list, &tcp_input_retrans_list, &tcp_output_retrans_list, oppo_nw_wakeup, true);
	reset_count(service_wakeup_times, &tcp_input_list, &tcp_output_list, &tcp_input_retrans_list, &tcp_output_retrans_list, oppo_nw_wakeup);
	app_wakeup_monitor_list.count = 0;
	memset(app_wakeup_monitor_list.set, 0x0, sizeof(app_wakeup_monitor_list.set));
	if (unsl) {
...
static void nwpower_unsl_mdaci() {
	print_qrtr_wakeup(true, mdaci_service_wakeup_times, oppo_mdaci_nw_wakeup, true);
	print_ipa_wakeup(true, &mdaci_tcp_input_list, &mdaci_tcp_output_list,
					&mdaci_tcp_input_retrans_list, &mdaci_tcp_output_retrans_list, oppo_mdaci_nw_wakeup, true);
	reset_count(mdaci_service_wakeup_times, &mdaci_tcp_input_list, &mdaci_tcp_output_list,
				&mdaci_tcp_input_retrans_list, &mdaci_tcp_output_retrans_list, oppo_mdaci_nw_wakeup);
...
static int nwpower_netlink_rcv_msg(struct sk_buff *skb, struct nlmsghdr *nlh, struct netlink_ext_ack *extack) {
	...
	case NW_POWER_REQUEST_BLACK_REJECT:
		nwpower_unsl_blacklist_reject();
		printk("[oppo_nwpower] netlink: unsl_blacklist_reject");
		break;
	case NW_POWER_REQUEST_APP_WAKEUP:
		nwpower_unsl_app_wakeup();
		printk("[oppo_nwpower] netlink: unsl_app_wakeup");
		break;
	case NW_POWER_REQUEST_MDACI:
		nwpower_unsl_mdaci();
		printk("[oppo_nwpower] netlink: unsl_mdaci");
		break;
	default:
		return -EINVAL;
	}

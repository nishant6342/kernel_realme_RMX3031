...
{
	return (sched_assist_scene(SA_SLIDE) ||  sched_assist_scene(SA_LAUNCHER_SI) || sched_assist_scene(SA_INPUT) || sched_assist_scene(SA_ANIM)) &&
		(is_heavy_ux_task(p) || is_sf(p)) &&
		oplus_task_misfit(p, cpu);
}

static void inline slide_set_start_cpu(struct root_domain *rd, struct task_struct *p, int *start_cpu)
{
	if ((sched_assist_scene(SA_SLIDE) ||  sched_assist_scene(SA_LAUNCHER_SI) || sched_assist_scene(SA_INPUT) || sched_assist_scene(SA_ANIM)) && (is_heavy_ux_task(p) ||  is_sf(p))) {
...
{
	u64 tmpload = *load;
	if (sched_assist_scene(SA_SLIDE) ||  sched_assist_scene(SA_LAUNCHER_SI) || sched_assist_scene(SA_INPUT) || sched_assist_scene(SA_ANIM)) {
